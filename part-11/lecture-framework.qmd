...
format:
    ...
    revealjs:
        ...
        code-link: true
        preview-links: auto
        footer: " {{< meta title >}} | {{< meta author >}} | [Home](lecture-framework.qmd)"
...
$\sum_{\tau \in [\max(t-3,1), t-1]} v_{3,\tau} \leq (1 - v_{4,t}) M \quad \forall t \in \mathcal{T}$

### How it Works:

1. **When $v_{4,t} = 1$** (generator 4 starts at time $t$):
   - The right-hand side becomes $(1-1) M = 0$
   - Both constraints force $\sum v_{3,\tau} = 0$ in the forward and backward windows
   - Generator 3 cannot start within 3 periods before or after $t$
...
### Example Calculation:
For the startup timing constraint, a safe choice is:

- $M = |\mathcal{T}|$ (total number of time periods)
- This upper-bounds the left-hand side even if every $v_{3,\tau}$ in the window equals 1
- Tighter values are possible (e.g. $M = 4$ for a 3-period window), but $|\mathcal{T}|$ is always valid
...
**Standard MILP approach:**

$p_g \leq p_g^{\max} (u_g^{min} + u_g^{max})$

$p_g \geq p_g^{min} u_g^{min} + p_g^{max} u_g^{max}$

$u_g^{off} + u_g^{min} + u_g^{max} = 1$
...
### 3. Reformulation
Sometimes big M constraints can be replaced by equivalent formulations with additional variables or constraints that are more efficient or numerically stable.

**Example**: Minimum up-time constraints are often written directly (without big M):

$\sum_{t'=t}^{t+T^{min}-1} u_{g,t'} \geq T^{min} v_{g,t} \quad \forall g \in \mathcal{G}^{UC}, t \in \mathcal{T}$

**Alternative reformulation** (tracking remaining up-time with continuous variables):
```julia
...
- $\mathcal{G}^{Dem}$ - Subset of technologies linked to demand satisfaction (each with associated carrier $e_g$)
- $\mathcal{G}^{Sup}$ - Subset of supply technologies (each with associated carrier $e_g^{sup}$)
...
::: {.callout-tip}
**Teaching tip:** Specialize the framework by example — map electrolyzers, batteries, and grid connections to entries in $\mathcal{G}$ and $\mathcal{E}$, then deactivate unused constraint blocks (UC, supply, etc.).
:::
...
- $P_g^{min}$ - Minimum production level of technology $g$ when online
- $\pi_g^{dem}$ - Value of satisfying demand associated with technology $g \in \mathcal{G}^{Dem}$ [EUR/unit]
- $Q_g^{nom}$ - Nominal supply quantity for technology $g \in \mathcal{G}^{Sup}$
- $\tau_g$ - Tax or levy on purchases related to supply technology $g \in \mathcal{G}^{Sup}$ [EUR/unit]
- $\eta_g^{ch}$, $\eta_g^{dis}$ - Charging and discharging efficiency of storage technology $g$
- $\delta_g$ - Self-discharge rate of storage technology $g$
- $f_{g,t,\omega}$ - Capacity factor of variable technology $g$ at time $t$ in scenario $\omega$
...
- $SOC_g^{init}$ - Initial state of charge of storage technology $g$ (also used for $t=1$ in the balance equation via $soc_{g,0,\omega} = SOC_g^{init}$)
- $SOC_g^{max}$ - Optional technical upper bound on storage energy capacity
...
$-\sum_{\omega \in \Omega} \pi_{\omega}^{prob} \sum_{g \in \mathcal{G}^{Sup}} \sum_{t \in \mathcal{T}} \tau_g f_{e_g^{sup},t,\omega}^{buy}$
...
$x_{g,t,\omega}^{total} \leq M u_{g,t,\omega} \quad \forall g \in \mathcal{G}^{UC}, t \in \mathcal{T}, \omega \in \Omega$

$P_g^{min} \leq x_{g,t,\omega}^{total} + M (1 - u_{g,t,\omega}) \quad \forall g \in \mathcal{G}^{UC}, t \in \mathcal{T}, \omega \in \Omega$

where $M$ is a large constant (big M), typically $M = P_g^{nom}$.

::: {.callout-note}
The on/off and minimum-power constraints apply only to unit-commitment technologies ($\mathcal{G}^{UC}$). When $u_{g,t,\omega} = 1$, the third constraint enforces $x_{g,t,\omega}^{total} \geq P_g^{min}$. When $u_{g,t,\omega} = 0$, the second constraint forces $x_{g,t,\omega}^{total} = 0$.
:::
...
$0 \leq soc_{g,t,\omega} \leq SOC_g^{nom} \quad \forall g \in \mathcal{G}^S, t \in \mathcal{T}, \omega \in \Omega$

### Initial and Final State
$soc_{g,1,\omega} = SOC_g^{init} \quad \forall g \in \mathcal{G}^S, \omega \in \Omega$

$soc_{g,|\mathcal{T}|,\omega} = SOC_g^{init} \quad \forall g \in \mathcal{G}^S, \omega \in \Omega$ *(cyclic horizon)*
