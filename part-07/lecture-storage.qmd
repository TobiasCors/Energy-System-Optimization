...
For non-linear fuel consumption curves, we use a **piecewise linear approximation** with a **Special Ordered Set of type 2 (SOS2)** formulation:

1. Define breakpoints on the fuel-consumption curve
2. Connect breakpoints with linear segments
3. Use continuous weights $\lambda$ on breakpoints and binary variables $\delta$ to select the active segment

### Breakpoints, Segments, and Parameters

- $\mathcal{B} = \{1,2,\ldots,K\}$ — set of **breakpoints** indexed by $k$
- $\mathcal{L} = \{1,2,\ldots,K-1\}$ — set of **segments** indexed by $s$; segment $s$ connects breakpoint $s$ to $s+1$
- $P_{g,k}$ — power output at breakpoint $k$ for generator $g$ [MW]
- $F_{g,k}$ — fuel consumption at breakpoint $k$ for generator $g$ [MWh]

The breakpoints lie on (or slightly above) the convex fuel curve; the red chords in the figure are the linear segments.

### Decision Variables

- $\lambda_{g,t,k} \geq 0$ — weight on breakpoint $k$ (convex-combination variable)
- $\delta_{g,t,s} \in \{0,1\}$ — $1$ if segment $s$ is active for generator $g$ at time $t$

### Interpolation (same weights for $p$ and $f$)

$p_{g,t} = \sum_{k \in \mathcal{B}} \lambda_{g,t,k} P_{g,k} \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

$f_{g,t} = \sum_{k \in \mathcal{B}} \lambda_{g,t,k} F_{g,k} \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

### SOS2 Constraints

**Convex combination**:

$\sum_{k \in \mathcal{B}} \lambda_{g,t,k} = 1 \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

**One active segment**:

$\sum_{s \in \mathcal{L}} \delta_{g,t,s} = 1 \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

**Link weights to segments** (at most two adjacent $\lambda$'s may be positive):

$\lambda_{g,t,1} \leq \delta_{g,t,1} \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

$\lambda_{g,t,k} \leq \delta_{g,t,k-1} + \delta_{g,t,k} \quad \forall g \in \mathcal{G}, t \in \mathcal{T}, k \in \{2,\ldots,K-1\}$

$\lambda_{g,t,K} \leq \delta_{g,t,K-1} \quad \forall g \in \mathcal{G}, t \in \mathcal{T}$

::: {.callout-note}
When $\delta_{g,t,s}=1$, only $\lambda_{g,t,s}$ and $\lambda_{g,t,s+1}$ can be positive. The operating point is a convex combination of the two endpoints of segment $s$, so $(p_{g,t}, f_{g,t})$ lies on the corresponding chord. This is the standard MILP formulation for approximating a convex fuel curve.
:::
...
<div style="position: relative; width: 600px; height: 400px; margin: 20px auto;">
    ...
    <!-- Delta variables (one per segment; segment 2 active) -->
    <div style="position: absolute; left: 100px; bottom: 20px;">
        <span class="math inline">\(\delta_{g,t,1}=0\)</span>
    </div>
    <div style="position: absolute; left: 200px; bottom: 20px;">
        <span class="math inline">\(\delta_{g,t,2}=1\)</span>
    </div>
    <div style="position: absolute; left: 300px; bottom: 20px;">
        <span class="math inline">\(\delta_{g,t,3}=0\)</span>
    </div>
    
    <!-- Lambda weights on breakpoints (segment 2: λ₁ and λ₂) -->
...
2. Four breakpoints ($k=1,\ldots,4$) and three segments ($s=1,2,3$)
3. Example operating point (green) at ($p_{g,t}$, $f_{g,t}$)
4. Weights $\lambda_{g,t,k}$ on **breakpoints** and binaries $\delta_{g,t,s}$ on **segments**

The green point lies on **segment 2** (between $P_{g,1}$ and $P_{g,2}$), with:

- $\delta_{g,t,2}=1$ and $\delta_{g,t,s}=0$ for $s \neq 2$
- $\lambda_{g,t,1} + \lambda_{g,t,2} = 1$ and $\lambda_{g,t,1}, \lambda_{g,t,2} > 0$ (interpolation along the chord)
- $\lambda_{g,t,3} = \lambda_{g,t,4} = 0$
:::

### Convexity and Why SOS2 Works

The **fuel consumption** curve $f(p)$ is convex (for convex efficiency curves, $f = p/\eta$ is convex in $p$). For a convex function, any point on a chord between two breakpoints lies **above** the true curve; the MILP uses the chord as an outer approximation (typically a slight overestimate of fuel use).

The SOS2 constraints ensure:

1. Only one segment is active ($\sum_s \delta_{g,t,s} = 1$)
2. At most two adjacent breakpoint weights are positive
3. Interpolation cannot jump across non-adjacent breakpoints

In the example above, with $\delta_{g,t,2}=1$:

$p_{g,t} = \lambda_{g,t,1} P_{g,1} + \lambda_{g,t,2} P_{g,2}, \quad
f_{g,t} = \lambda_{g,t,1} F_{g,1} + \lambda_{g,t,2} F_{g,2}$

::: {.callout-tip}
While the **efficiency** curve of a thermal unit is often concave in $p$, we approximate the **fuel consumption** curve $f(p)$, which is convex. That convexity justifies this chord-based MILP formulation. Tighter UC formulations (e.g. incremental heat input) are discussed in @morales2013tight.
:::
