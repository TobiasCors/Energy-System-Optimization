...
function solve_stochastic_design(dictStorage, dictWindTurbines, dictPV, dictScenarios, dictGrid, max_budget)
    ...
    pvaf = Dict(
        "storage" => (1 - 1/(1 + dictStorage["storage"].discount_rate)^dictStorage["storage"].lifetime) / dictStorage["storage"].discount_rate,
        "wind" => (1 - 1/(1 + dictWindTurbines["wind"].discount_rate)^dictWindTurbines["wind"].lifetime) / dictWindTurbines["wind"].discount_rate,
        "pv" => (1 - 1/(1 + dictPV["pv"].discount_rate)^dictPV["pv"].lifetime) / dictPV["pv"].discount_rate
    )
    ...
    @objective(model, Min, 
        ...
        sum(dictScenarios[ω].probability * (AC_grid_imp[ω] - AR_grid_exp[ω]) for ω in Ω)
    )
    ...
    @constraint(model, [ω in Ω],
        AC_grid_imp[ω] == sum(
            ...
            for t in T
        ) * 52.1429 # 52.1429 is the number of weeks in a year as we currently have a time horizon of one week
    )
